...
def run_command(command: str, cwd: str, proxy: Optional[str] = None) -> str:
    """Execute a shell command, inject proxy if provided, and handle rollbacks on failure."""
    print(f"Executing: [{command}] in {cwd}", flush=True)
    
    custom_env = os.environ.copy()
    if proxy:
        print(f"Routing traffic through proxy: {proxy}", flush=True)
        custom_env["HTTP_PROXY"] = proxy
        custom_env["HTTPS_PROXY"] = proxy
        custom_env["http_proxy"] = proxy
        custom_env["https_proxy"] = proxy
...
def detect_and_deploy(path: str, restart_cmd: Optional[str] = None, proxy: Optional[str] = None):
    """Manage the deployment lifecycle, handle dependencies via proxy, and execute rollbacks."""
    ...
    try:
        ...
        run_command("git pull", path, proxy=proxy)
        
        if os.path.exists(os.path.join(path, "package.json")):
            print("Framework detected: Node.js", flush=True)
            run_command("npm install", path, proxy=proxy)
            ...
            if os.path.exists(os.path.join(path, "next.config.js")) or os.path.exists(os.path.join(path, "next.config.mjs")):
                ...
                run_command("npm run build", path, proxy=proxy)
            ...
            run_command(f"{pip_cmd} install -r requirements.txt", path, proxy=proxy)

        if restart_cmd:
            print(f"Executing custom restart command: {restart_cmd}", flush=True)
            # سرویس‌های لوکال نیازی به عبور از پراکسی برای ری‌استارت ندارند
            run_command(restart_cmd, path, proxy=None)
        else:
